...
void parser(FILE *f, HashMap *hashMap,MapperWorkQueue* mapperQ){
...
void mapper(MapperWorkQueue* mapperQ, ReductionWorkQueue* reductionQ, int index) {
...
void reducer(ReductionWorkQueue* reductionQ, HashMap* hashMap, int index) {
    ReductionWorkElement* temp = reductionQ[index].head;
    while (temp != NULL) {
    ...
    for(tid = 1; tid < NUMPROCESS; tid++){
        ...
        if(tid > 0 && tid <= NUM_READERS) {
            ...
            if(input == NULL){
                ...
                parser(input, hashMap,mapperQ);
                readerIsDone -= 1;
            }
        }
        else if(tid > NUM_READERS && tid <= NUM_READERS + NUM_MAPPERS) {
            //MAPPER
            mapper(mapperQ,reductionQ,tid - (NUM_MAPPERS + 1));
            ...
            //REDUCER
            reducer(reductionQ,hashMap,tid-(NUM_READERS + NUM_MAPPERS + 1));
        }
    }
